...
def gen_gridded_space_DET(feature_dict, oneFeaturePerPoint=True, N=N_default, xlim=xlim_default, ylim=ylim_default):
    ...
    OUTPUT:
    -(X, Y, Z) tuple corresponding to a meshgrid of X, Y values and Z, which is a dict with N^2 keys
    and a value corresponding to a tuple that records the indicies where each feature type is present
    """
...
def plot_truth_features(feature_dict, xlim=xlim_default, ylim=ylim_default, oneFeature=False):
    '''
    INPUT:
    -features_dict: dictionary with key of feature type with the following value:
        --Value is a tuple with the following elements (length of this tuple corresponds to the number of features of this type):
...
def plot_sampled_features(Z, feature_dict, N=N_default, xlim=xlim_default, ylim=ylim_default, plotNoFeatures=True,
